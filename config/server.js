...
app.get(config.ApiBaseUrl + "user/listusers",
    authMidd.validateJWt,
    authMidd.validateAdmin,
    userControl.getUsers);

app.post(config.ApiBaseUrl + "user/newuser",
    userControl.createUser);

app.post(config.ApiBaseUrl + "user/login",
    userControl.logUser)

//products routes

app.post(config.ApiBaseUrl + "product/newproduct",
    authMidd.validateJWt,
    authMidd.validateAdmin,
    productControl.addProducts);

app.get(config.ApiBaseUrl + "product/listproducts",
    authMidd.validateJWt,
    productControl.listProducts)

app.put(config.ApiBaseUrl + "product/updateproduct",
    authMidd.validateJWt,
    authMidd.validateAdmin,
    productControl.updateProducts)

app.delete(config.ApiBaseUrl + "product/removeproduct",
    authMidd.validateJWt,
    authMidd.validateAdmin,
...
app.post(config.ApiBaseUrl + "order/newOrder",
    authMidd.validateJWt,
    ordersControl.generateOrder)

app.patch(config.ApiBaseUrl + "order/updateOrder",
    authMidd.validateJWt,
    authMidd.validateAdmin,
    ordersControl.upadateOrder)

app.get(config.ApiBaseUrl + "order/myOrders",
    authMidd.validateJWt,
    ordersControl.listOrders)

app.get(config.ApiBaseUrl + "order/allOrders",
    authMidd.validateJWt,
    authMidd.validateAdmin,
